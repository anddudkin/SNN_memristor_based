...
public:
    ...
    int forward(const float* input_spikes, float* spikes_out) {
        ...
        // 2. Считаем ДО WTA (для статистики)
        static int total_raw = 0;


        // 3. Winner-takes-all только среди spiking
        int winner = -1;
        float max_v = -1.0f;
        for(int j = 0; j < N_NEURONS; j++) {
        ...
        // 4. WTA: только 1 победитель
        if(winner >= 0) {
            std::fill(spikes_out, spikes_out + N_NEURONS, 0.0f);
            spikes_out[winner] = 1.0f;

            // Торможение остальных
            for(int j = 0; j < N_NEURONS; j++) {
    ...
    //std::cout << "Raw spikes: " << raw_spikes << ", WTA: " << (winner>=0 ? 1 : 0)
...
int main() {
    ...
    for(int img = 0; img < N_IMAGES; img++) {
        std::cout << "Изображение " << img + 1 << "/" << N_IMAGES << std::endl;

        for(int t = 0; t < TIME_STEPS; t++) {
            generate_image_spikes(input_spikes, 0.3f);

        int tick_input_spikes = 0;
        for(int i = 0; i < N_INPUT; i++) {
            if(input_spikes[i] > 0) tick_input_spikes++;
        }


        std::cout << "  Тик " << t+1 << ": " << tick_input_spikes
                  << " входных спайков | " << std::endl;
    ...
    std::cout << "\n✅ ИТОГО WTA спайков: " << total_wta_spikes << std::endl;
    std::cout << "Активность: " << (float)total_wta_spikes / (N_IMAGES * TIME_STEPS * N_NEURONS) << std::endl;
