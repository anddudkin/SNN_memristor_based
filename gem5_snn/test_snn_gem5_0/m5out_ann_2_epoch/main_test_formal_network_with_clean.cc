...
#define NUM_TRAIN 60000   // Количество тренировочных примеров
...
void generate_random_data() {
    //printf("Генерация случайных данных (%d примеров)...\n", NUM_TRAIN);
    srand(time(NULL));

    for (int i = 0; i < NUM_TRAIN; i++) {
        // Генерируем случайный лейбл от 0 до 9
        train_labels[i] = rand() % OUTPUT_SIZE;

        // Генерируем случайные пиксели (0-1) для каждого изображения
